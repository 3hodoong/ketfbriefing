...
MAX_SIGNALS = 6        # 탐지 최대 개수 (프롬프트에는 상위 3개만 전달)
...
★★ 분량 통제가 가장 중요합니다 ★★
전체 응답은 PART 1과 PART 2로 구성되며, PART 2의 B안까지 반드시 완성해야 합니다.
분량이 부족해서 B안을 못 쓰는 일이 절대 없도록, 앞부분을 짧게 쓰세요.
PART 1은 전체의 40%, PART 2는 60%를 쓴다고 생각하세요.

═══ PART 1. 판단 재료 ═══
신호가 여러 개여도 상위 3개까지만 다루세요. 나머지는 무시합니다.
각 신호마다 아래 형식으로, 한 신호당 최대 4줄입니다.

N. (분류) 종목/항목명
   관찰: 한 줄
   해석: 가능한 방향 2가지를 한 줄에 · 로 구분해서
   확인: 무엇을 더 봐야 하는지 한 줄

건조하게. 문장을 늘리지 마세요. 전문가가 읽는 메모입니다.

═══ PART 2. 블로그 코멘트 초안 ═══
가장 이야깃거리가 되는 신호를 골라 초안 2개를 씁니다. A안과 B안 모두 반드시 완성하세요.

★ 각 초안은 4문장. 4문장을 넘기지 마세요.
A안을 길게 써서 B안 분량을 잡아먹는 것이 가장 흔한 실패입니다.

★ 각도 선택
신호의 성격에 맞는 각도를 고르세요. 모든 신호를 지수나 팩터 이야기로 끌고 가지 마세요.
개별 종목의 급등락을 억지로 지수·ETF·리밸런싱 프레임에 연결하지 마세요.
그런 연결은 그 요인이 실제로 작동했다는 근거가 데이터에 있을 때만 하세요.

각도를 고르는 기준:
- 개별 종목의 큰 움직임 → 그 사안이 앞으로 미국·한국 시장에 어떤 영향을 줄 가능성이 있는지.
  해당 종목의 밸류체인, 경쟁 구도, 같은 업종 한국 기업에 미칠 파급.
- 매크로 지표 간 엇갈림 → 경기·금리 국면에 대한 함의
- 여러 종목·섹터에 걸친 광범위한 움직임 → 이때만 팩터·지수 관점이 적절
- 거래대금 이상 → 수급 관점
- 지역 간 디커플링 → 한국 시장 연결

A안과 B안은 서로 다른 각도, 가능하면 서로 다른 신호를 다루세요.

각 초안 작성 규칙:
- 4문장. 각 문장은 길어도 두 줄을 넘기지 마세요.
- 1문장: 관찰된 사실 (숫자 포함)
- 2문장: 왜 그랬을 가능성이 있는지
- 3문장: 그래서 이게 앞으로 무엇을 의미할 수 있는지 (한국 연결이 자연스러우면 여기서)
- 4문장: 앞으로 무엇을 보면 되는지
- 문체는 조심스럽게. "~로 보입니다", "~일 가능성이 있습니다", "~는 아직 단정하기 어렵습니다".
- 전문용어는 그대로 쓰되 처음 나올 때 괄호로 짧게 풀이. 한 초안에 1개까지만.
- 특정 종목·ETF의 매수·매도 권유 절대 금지. 목표가, 미래 수익률 전망 금지.
...
- 뉴스 헤드라인은 시점이 맞는 것만 조심스럽게 연결. 억지로 엮지 말 것.
...
def main():
    ...
    if signals:
        # 상위 3개만 전달 — 신호가 많으면 PART 1이 길어져 B안이 잘림
        top_signals = fmt_signals(signals[:3])
        material = call_claude(
            ...
            f"=== 참고: 같은 날 시장 데이터 ===\n{data_block}\n\n"
            f"=== 참고: 최근 뉴스 헤드라인 ===\n{news_block[:2000]}\n\n"
            f"PART 1(상위 3개, 신호당 4줄)과 PART 2(A안·B안 각 4문장)를 작성해 주세요.\n"
            f"B안까지 반드시 완성하세요.",
            max_tokens=4000,
        )

        # 절단 감지 — 재호출하지 않고 안내만 붙임 (비용 절약)
        tail = material.split("B안")[-1] if "B안" in material else ""
        if "B안" not in material or len(tail) < 80:
            print("  [warn] B안 누락/절단 감지")
            material += (
                "\n\n─────────────\n"
                "※ B안이 분량 한계로 잘렸습니다. PART 1의 다른 신호를 참고해 직접 써보세요."
            )
    else:
        material = (
            "오늘은 통계적으로 특이한 움직임이 감지되지 않았습니다.\n"
